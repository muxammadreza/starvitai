---
name: starvit-rule-03-rules-workflows-integrity
description: These rules protect the team from “agent drift”, prompt injection, and supply-chain style tampering of the `.codex/` pack.
---

# Rules/workflows integrity and hardening

These rules protect the team from “agent drift”, prompt injection, and supply-chain style tampering of the `.codex/` pack.

## 1) Trust boundary: `.codex/` is configuration, not content
- Treat `.codex/` as **security-critical**.
- Changes require:
  - PR review by Security + DevEx (CODEOWNERS recommended).
  - Updated `.codex/manifest.sha256` (see tooling below).
...
- After any change in `.codex/` (excluding `.codex/archive`):
  - run `.codex/tools/gen_manifest.sh`
...
## Tooling
- Generate manifest: `.codex/tools/gen_manifest.sh`
