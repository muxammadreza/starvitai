...
describe("auth routes", () => {
  it("redirects when state is missing or invalid", async () => {
    const request = new NextRequest("http://localhost:3001/auth/callback?code=abc&state=bad");
  ...
  it("redirects when code_verifier is missing", async () => {
    const request = new NextRequest("http://localhost:3001/auth/callback?code=abc&state=expected", {
      headers: {
  ...
  it("returns 401 when auth cookie is missing", async () => {
    const request = new NextRequest("http://localhost:3001/auth/me");
