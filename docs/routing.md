...
|---------|-------------|---------|------------|
| **Clinician UI** | `localhost:3001` | `clinician-staging.starvit.ca` | `clinician.starvit.ca` |
...
| **Medplum App** | `localhost:3000` | `medplum-staging.starvit.ca` | `medplum.starvit.ca` |
...
We use **Server-Side Proxying (Pattern A)** via **Route Handlers** to avoid baking API URLs into Docker images.
1. Frontend code calls `/api/...`.
2. Next.js App Router Route Handlers proxy this to `STARVIT_API_INTERNAL_URL`.
